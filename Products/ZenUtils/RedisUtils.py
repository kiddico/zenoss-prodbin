...
def parseRedisUrl(url):
    """Parses url and returns a dict of redis options suitable
    ...
    port = (
        16379
        if parsedUrl.group("port") is None
        else int(parsedUrl.group("port"))
    )
        ...
        try:
            for s in optStr.split(","):
                ...
                if s in ["host", "port", "db"]:
                    raise ValueError(
                        "malformed redis URL, "
                        "can not specify %r in options string"
                        % key
                    )
                options[key] = value
        except ValueError:
