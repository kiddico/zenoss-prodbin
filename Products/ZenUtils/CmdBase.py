...
from optparse import (
    OptionParser, OptionGroup, Option, SUPPRESS_HELP, NO_DEFAULT,
...
class CmdBase(object):
    ...
    def buildOptions(self):
        ...
        if self.doesLogging:
            ...
            group.add_option(
                '--logpath',
                dest='logpath', default=zenPath('log'), type='str',
        ...
        self.parser.add_option(
            "--genconf", action="store_true", default=False,
    ...
    def getGlobalConfigFileDefaults(self):
        ...
        # - it is duplicated in GlobalConfig.py
        """
        Parse a config file which has key-value pairs delimited by white space,
        and update the parser's option defaults with these values.
        """
                ...
                for line in file:
                    if line.lstrip().startswith('#') or line.strip() == '':
                        ...
                        try:
                            ...
                            # default value valid delimiters are space,
                            # ':' and/or '=' (see ZenUtils/config.py)
                            key, value = (
                                re.split(r'[\s:=]+', line.strip(), 1) + ['']
                            )[:2]
                        except ValueError:
                            lines.append(dict(
                                type='option', line=line, key=line.strip(),
                                value=None, option=None
                            ))
                        else:
                            option = self.parser.get_option('--%s' % key)
                            lines.append(dict(
                                type='option', line=line, key=key,
    ...
    def generate_configs(self, parser, options):
        ...
        @parameter parser: an optparse parser object which contains defaults
...
#%s %s""" % (comment, option_name, value)
    ...
    def generate_xml_table(self, parser, options):
        ...
        print '\n'.join([
            ...
            (
                '  <caption>{name} ',
                '<indexterm><primary>Daemons</primary>',
                '<secondary>{name}</secondary>',
                '</indexterm> options</caption>',
            ),
            '<tgroup cols="2">',
        ...
        for opt in getAllParserOptionsGen(parser):
            ...
            all_options = ''.join([
                '<literal>',
                re.sub(r'/', '</literal>,</para> <para><literal>', str(opt)),
                '</literal>'
            ])
            ...
            #
            # TODO: Determine the variable name used and
            # display the --option_name=variable_name
            #
...
""" % (all_options, comment, default_string)
                ...
                target = ''.join([
                   '=<replaceable>', opt.dest.lower(), '</replaceable>'
                ])
...
""" % (export_date, daemon_name)
        ...
        for opt in getAllParserOptionsGen(parser):
            ...
            #
            # TODO: Determine the variable name used
            # and display the --option_name=variable_name
            #
            if opt.action in ['store_true', 'store_false']:
                print ''.join([
                ...
                print (
                    '    <option id="%s" type="%s" default="%s" '
                    'target="%s" help="%s" />\n'
                ) % (
                    option_name, opt.type, quote(default_string),
                    target, quote(opt.help),
                )
