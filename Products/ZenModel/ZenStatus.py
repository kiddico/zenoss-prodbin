...
class ZenStatus(Persistent):
    ...
    def getStatusString(self):
        ...
        return (
            self.conversions[self.status]
            if self.status in self.conversions
            else self.status
        )
...
class ZenAvailability(ZenStatus):
    ...
    def getAvailPercent(self, start, end=None):
        """get availability for a date range as a float between
        100.0 and 0.0
        """
        if self.status < 0:
            return -1
    ...
    def _getYearlyData(self, year):
        ...
        if year not in self.yearlydata:
...
class YearlyDownTime(Persistent):
    ...
    def __init__(self):
        days = 365
        if calendar.isleap(DateTime().parts()[0]):
            days += 1
    ...
    def getDownTime(self, start=None, end=None):
        """get the down time in seconds over a date range,
        dates are DateTime
        """
        if end is None:
